...
describe('Full Migration Integration', () => {
  it('should migrate complex worker config', async () => {
    ...
    expect(result.workers[0].props.compatibilityFlags).toContain('nodejs_compat');

    // Resources
    expect(result.resources).toHaveLength(6); // 2 KV + 1 R2 + 1 D1 + 1 Queue + 1 DO
    expect(result.resources.filter((r) => r.type === 'KVNamespace')).toHaveLength(2);
    expect(result.resources.filter((r) => r.type === 'R2Bucket')).toHaveLength(1);
    expect(result.resources.filter((r) => r.type === 'D1Database')).toHaveLength(1);
    expect(result.resources.filter((r) => r.type === 'Queue')).toHaveLength(1);
    expect(result.resources.filter((r) => r.type === 'DurableObjectNamespace')).toHaveLength(1);
  ...
  it('should handle environment-specific configs', async () => {
    ...
    expect(prodResult.resources.filter((r) => r.type === 'R2Bucket')).toHaveLength(1);
  });
  ...
  it('should handle resource adoption settings', async () => {
    ...
    expect(adoptResult.resources.every((r) => r.type !== 'DurableObjectNamespace' ? r.adopt : true)).toBe(true);

    const noAdoptResult = transformer.transform(parsed.config, { adopt: false });
    expect(noAdoptResult.resources.every((r) => r.type !== 'DurableObjectNamespace' ? !r.adopt : true)).toBe(true);
  });
});
