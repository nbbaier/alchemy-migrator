...
export interface AlchemyResource {
  type: 'KVNamespace' | 'R2Bucket' | 'D1Database' | 'Queue' | 'DurableObjectNamespace';
...
export class ResourceMapper {
   ...
   * Map KV Namespaces
   */
  mapKVNamespaces(kvNamespaces?: WranglerConfig['kv_namespaces']): AlchemyResource[] {
    ...
    return kvNamespaces.map((kv, index) => {
      ...
      const title = this.options.preserveNames && kv.id
   ...
   * Map D1 Databases
   */
  mapD1Databases(d1Databases?: WranglerConfig['d1_databases']): AlchemyResource[] {
   ...
   */
  mapDurableObjects(durableObjects?: WranglerConfig['durable_objects']): AlchemyResource[] {
    if (!durableObjects?.bindings || durableObjects.bindings.length === 0) return [];
  ...
  private generateVariableName(prefix: string, id: string, index: number): string {
...
export class ConfigTransformer {
  transform(
    ...
    const bindings = BindingTransformer.transformBindings(targetConfig, bindingContext);
    ...
    const worker: AlchemyWorker = {
      ...
      eventSources: this.transformQueueConsumers(targetConfig.queues?.consumers, resourceMap),
    };
...
import type { AlchemyConfig, AlchemyResource, AlchemyWorker } from '../types/alchemy';
...
import { discoverWranglerConfig, detectProjectContext } from '../../utils/file-utils';
...
export function validateConfig(config: WranglerConfig): ValidationResult {
  ...
  if (config.site) {
    warnings.push('Static site configuration should use Alchemy Assets resource');
  }
...
describe('Parser', () => {
  it('should parse TOML configuration', async () => {
    const result = await parseWranglerConfig('./tests/fixtures/simple-worker.toml');
  ...
  it('should parse JSON configuration', async () => {
    const result = await parseWranglerConfig('./tests/fixtures/simple-worker.json');
  ...
  it('should normalize environment configs', async () => {
    const result = await parseWranglerConfig('./tests/fixtures/complex-worker.toml');
    
    expect(result.config.environments).toHaveProperty('production');
    expect(result.config.environments.production.name).toBe('complex-worker-prod');
  });
});
```
...
describe('End-to-end migration', () => {
  it('should migrate simple worker successfully', async () => {
    ...
    try {
      ...
      // Verify content
      const alchemyConfig = readFileSync(join(tmpDir, 'alchemy.run.ts'), 'utf-8');
...
export function mergeEnvironment(options: EnvMergeOptions): WranglerConfig {
  ...
  if (envConfig.compatibility_date !== undefined) merged.compatibility_date = envConfig.compatibility_date;
  
  // Array replacements (env replaces base entirely)
  if (envConfig.routes !== undefined) merged.routes = envConfig.routes;
  if (envConfig.kv_namespaces !== undefined) merged.kv_namespaces = envConfig.kv_namespaces;
  if (envConfig.r2_buckets !== undefined) merged.r2_buckets = envConfig.r2_buckets;
  if (envConfig.d1_databases !== undefined) merged.d1_databases = envConfig.d1_databases;
  
  // Object merges
  if (envConfig.observability !== undefined) {
    merged.observability = { ...merged.observability, ...envConfig.observability };
  }
