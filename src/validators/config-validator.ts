...
export function validateConfig(config: WranglerConfig): ValidationResult {
  ...
  config.kv_namespaces?.forEach((kv: any) => addBinding(kv.binding, 'KV'));
  config.r2_buckets?.forEach((r2: any) => addBinding(r2.binding, 'R2'));
  config.d1_databases?.forEach((db: any) => addBinding(db.binding, 'D1'));
  config.queues?.producers?.forEach((q: any) => addBinding(q.binding, 'Queue'));
